...
<h3>커피 주문 하는 법</h3>
<p>주문하고 싶은 커피의 수량을 적어서 제출하시오. </p>
<p>커피마다 걸리는 시간을 참고하시오.  </p>
</div>
...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        for($i=0; $i<13; $i++){
            $coffee[$i] = htmlspecialchars($_POST["coffee".$i]);
        }
    }
    ?>
...
<button class="w3-btn w3-padding-large" style="font-size:20px; background-color:#00004d">Submit</button>
</li>
</form>
</ul>
</div>
...
<!-- Grid -->
<div class="w3-row-padding w3-center" id="about" style="background-color:#ffffe6"><br>
<b class="w3-xlarge w3-bottombar w3-border-dark-grey w3-padding-15" style="color:#cc0066; font-size:40px">주문 내역서</b>
		<br><br>
<?php
    ...
    $file = fopen("OrderList.json", "w") or die("Unable to open file!");
    fwrite($file, "{\n");
    ...
    for($i=0; $i<13; $i++){
        if($coffee[$i]!=0 and is_numeric($coffee[$i])){
            ...
            fwrite($file,
                   json_encode(array("cost"=>"$cost[$i]", "id"=>"$i", "name"=>"$menuName[$i]", "quantity"=>"$coffee[$i]")).",\n");
        }
    }
    fwrite($file, "}");
    fclose($file);
...
<div class="w3-row-padding w3-center" id="contact" style="background-color:#ffffe6" ><br>
<b class="w3-xlarge w3-bottombar w3-border-dark-grey w3-padding-15" style="color:#cc0066; font-size:40px">주문 결과</b>
		<br><br>
		<!--json파일 받아와서 출력하기-->
...
<!-- Footer -->
<footer class="w3-container w3-padding-20 w3-light-grey w3-center"><br>
<h5>20135179_김다은 : 20113336_채한울 : 20113313_이창현</h5>
